...
void DrawDoorMetatileAt(int x, int y, u16 *tiles)
{
    ...
    if (offset >= 0)
    {
        DrawMetatile(0xFF, tiles, offset); // modified for triple layer metatiles
        sFieldCameraOffset.copyBGToVRAM = TRUE;
    }
}
...
static void DrawMetatile(s32 metatileLayerType, const u16 *tiles, u16 offset) // modified for triple layer metatiles
{

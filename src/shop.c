    ...
    for (j = 0; j < 10; j++)
    {
        for (i = 0; i < 15; i++)
        {
            ...
            if (BuyMenuCheckForOverlapWithMenuBg(i, j) == TRUE)
                metatileLayerType = METATILE_LAYER_TYPE_NORMAL; // modified for triple layer metatiles
            else
...
static bool8 IsMetatileLayerEmpty(const u16 *src) // new function for triple layer metatiles
{
    u32 i = 0;
    for (i = 0; i < 4; ++i)
    {
...
static void BuyMenuDrawMapMetatile(s16 x, s16 y, const u16 *src, u8 metatileLayerType) // modified for triple layer metatiles
{
    u16 offset1 = x * 2;
    u16 offset2 = y * 64;
